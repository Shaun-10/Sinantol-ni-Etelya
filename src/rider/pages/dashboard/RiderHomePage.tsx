...
export default function RiderHomePage() {
    ...
    const loadData = async () => {
      ...
      setStats([
        ...
        },
        {
          ...
          label: "Today Pending Deliveries",
        },
        {
          icon: FiUser,
        ...
        {
          icon: FiMapPin,
          value: toCurrency(totalCod),
          label: "COD Collected Today",
        },
      ]);

      const topOrders: Order[] = inProgress
        .slice(0, 9)
  ...
  return (
        ...
        <>
          <div className="bg-gray-100 border-t border-gray-300 p-2 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2.5">
              <button
                type="button"
                className="w-7.5 h-7.5 border-none bg-transparent text-[#101810] grid place-items-center text-[1.2rem] cursor-pointer hover:opacity-75"
          ...
          <div className="px-2.5 py-2">
            ...
            <h2 className="m-0 mt-0.5 mb-3.5 text-3xl leading-tight text-rider-text-main font-bold">
              {greetingName}
            </h2>
          </div>
        </>
    ...
    >
      {dataIssue ? (
        <p className="m-0 mb-3 rounded border border-[#e2b4b4] bg-[#fff0f0] px-3 py-2 text-xs text-[#9b1d1d]">
          {dataIssue}
        </p>
      ) : null}
        ...
        {stats.map((item) => {
          ...
          return (
            ...
            >
              ...
              <strong className="text-[2rem] text-rider-green-bold leading-tight">
                {isLoading ? "-" : item.value}
              </strong>
              <span className="text-[0.76rem] font-bold text-[#222]">
                {item.label}
              </span>
            </article>
          );
      ...
      <section className="mt-5">
        <div className="flex items-center justify-between mb-2.5">
          <h3 className="m-0 text-[1.2rem] text-[#1f281f] font-bold">
            Active Deliveries
          </h3>
          <button
            type="button"
        ...
        <div className="flex flex-col gap-3">
          {activeOrders.map((order) => (
            <button
              ...
              onClick={() =>
                navigate(
                  `/rider/deliveries/details?id=${encodeURIComponent(order.id)}`,
                )
              }
            >
              <div className="min-w-0 flex-1">
                <h4 className="m-0 text-base text-rider-text-main font-bold">
                  {order.customer}
                </h4>
