...
export default function RiderDeliveryPaymentPage() {
  ...
  return (
    ...
    >
      {isLoading ? (
        <p className="m-0 mb-3 text-sm text-[#5b645c]">Loading delivery...</p>
      ) : null}
      ...
      <article className="bg-rider-details-card rounded-xl p-3 mb-3">
        ...
        <h2 className="m-0 mt-1.75 mb-1 text-[#0c631f] text-[2rem] font-bold">
          {delivery?.customer || "-"}
        </h2>
        ...
        <p className="m-1 text-sm text-[#15a63d] font-black">
          {delivery?.contact || "-"}
        </p>
      ...
      {/* Order Items Card */}
      <article className="bg-rider-details-card rounded-xl p-3 mb-3">
        <h3 className="m-0 mb-1.5 text-[#4b534d] text-sm font-bold">
          ORDER ITEMS
        </h3>
        <ul className="m-0 pl-4.5">
          {(delivery?.items.length ? delivery.items : ["-"]).map(
            (item, index) => (
              <li
                key={`${item}-${index}`}
                className="mb-1 text-[#3d453f] text-sm"
              >
                {item}
              </li>
            ),
        ...
        <div className="mt-2 flex items-center justify-between">
          <span className="text-[#4d564e] text-sm">
      ...
      <article className="bg-rider-details-card rounded-xl p-3 mb-3">
        ...
        <p className="m-0 text-center text-[#2d322f] font-bold">
          {delivery?.customer || "-"}
        </p>
        <p className="m-0 text-center text-[#616a62] text-sm">Scan to pay</p>
        <p className="m-0 mb-2 text-center text-[#0d7420] text-[1.8rem] font-black">
          {delivery ? toCurrency(delivery.amount) : "-"}
        </p>
        <button
          type="button"
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <button
          type="button"
          className="border-none rounded-[10px] px-3 py-3 font-black bg-[#ef8f8f] text-[#f21f1f] cursor-pointer hover:opacity-90"
