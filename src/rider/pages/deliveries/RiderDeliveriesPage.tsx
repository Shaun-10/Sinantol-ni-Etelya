  ...
  return (
    <RiderAppLayout>
      <section className="w-full">
        <h2 className="m-0 mb-3.5 text-[#166723] text-[2.2rem] font-bold">
          Deliveries
        </h2>
        ...
        <article
          ...
          onClick={() =>
            activeDelivery.id &&
            navigate(
              `/rider/deliveries/details?id=${encodeURIComponent(activeDelivery.id)}`,
            )
          }
        >
          <p className="m-0 text-rider-next-stop-label text-xs font-black">
            NEXT BEST STOP - {activeDelivery.distance}
          </p>
          <h3 className="m-0 mt-1.25 text-rider-next-stop-green text-[1.9rem] font-bold">
            {activeDelivery.customer}
          </h3>
          <p className="m-0.5 text-[#5b615c] text-sm">
        ...
        {/* Active Section */}
        <div className="m-4 mb-2.5 text-[#505b50] text-base font-black">
          ...
          {activeDeliveries.map((delivery) => (
            <article
              ...
              onClick={() =>
                delivery.id &&
                navigate(
                  `/rider/deliveries/details?id=${encodeURIComponent(delivery.id)}`,
                )
              }
            >
              <div className="min-w-0">
                <h4 className="m-0 text-[1.35rem] text-[#116120] font-black">
                  {delivery.customer}
                </h4>
                <p className="m-0 mt-0.75 text-[0.76rem] text-[#5a625c]">
                  {delivery.address}
                </p>
                <div className="flex items-center gap-1.75 flex-wrap mt-1.75">
                  <span className="inline-flex items-center rounded-full px-2 py-1 text-[0.68rem] font-bold bg-rider-pill-yellow text-rider-pill-yellow-text">
              ...
              <div className="flex flex-col items-end justify-between flex-shrink-0">
                <span className="text-[#596259] text-sm">
                  {delivery.distance}
                </span>
                <button
                  type="button"
        ...
        {isLoading ? (
          <p className="m-4 text-sm text-[#576257]">Loading deliveries...</p>
        ) : null}
        ...
        <div className="flex flex-col gap-3">
          {completedDeliveries.map((item) => (
            <button
              ...
              onClick={() =>
                navigate(
                  `/rider/history/details?id=${encodeURIComponent(item.id)}`,
                )
              }
            >
              <div className="min-w-0 flex-1">
                <h4 className="m-0 text-[1.5rem] text-[#0f6420] font-black">
                  {item.customer}
                </h4>
