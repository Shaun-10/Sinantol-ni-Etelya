    ...
    <RiderAppLayout showBack backTo="/rider/history">
      ...
      <article className="bg-rider-details-card rounded-xl p-3 mb-3">
        <div className="flex items-center justify-between gap-2">
          ...
          <span
            className={`inline-flex items-center rounded-full px-2 py-1 text-[0.68rem] font-bold ${delivery?.status === "Delivered" ? "bg-[#cbf0ce] text-[#14722b]" : "bg-rider-pill-red text-rider-pill-red-text"}`}
          >
      ...
      <article className="bg-rider-details-card rounded-xl p-3">
        ...
        <strong
          className={`block mt-1 text-[1.05rem] ${delivery?.status === "Delivered" ? "text-[#0f7a23]" : "text-[#ff0909]"}`}
        >
          {delivery?.status === "Delivered"
            ? "Status: Completed"
            : `Reason: ${delivery?.failedReason || "-"}`}
        </strong>
      </article>
    </RiderAppLayout>
  );
